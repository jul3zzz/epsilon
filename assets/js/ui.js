...
(function (global) {
  ...
  /** Rendu d un avatar : emoji classique, lettre grecque stylisee (degrade, comme le logo ε), ou photo. */
  function avatarInner(valeur, taille) {
    ...
    if (/^[Α-ω]$/.test(valeur || '')) {
      return '<span class="epsilon-mark" style="font-size:' + Math.round((taille || 22) * 1.05) + 'px;line-height:1">' + valeur + '</span>';
    }
  ...
  /** Avatar a partager avec le systeme d amis (Firestore) : jamais une photo (trop lourd a synchroniser / pas prevu pour). */
  function avatarPourAmis(equipe) {
    var v = valAffichee(equipe, 'avatar');
  ...
  function amisDemarrer() {
    ...
    Amis.pret().then(function (ok) {
      ...
      var p = Store.joueur();
      if (ok && p && p.amisId) {
        Amis.inscrire(p.amisPseudo || p.amisId, avatarPourAmis(p.equipe)).then(function (r) {
  ...
  function agir(act) {
    ...
    switch (nom) {
      ...
      case 'amis-inscrire': {
        ...
        Amis.inscrire(champId.value, avatarPourAmis(p.equipe)).then(function (r) {
          if (r.ok) {
